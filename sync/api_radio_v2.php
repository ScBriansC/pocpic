...
function get_ws($parametro, $his = false){
	...
	$ws = @file_get_contents('http://192.168.11.23/ws_vh_mininter/Api?_sKey=RDO_X1485PU75634&_sAcc=dta_rdo_loc&_sID='.$_id);
	...
	if(isset($data_ws['sq_data']) && count($data_ws['sq_data'])>0){
		foreach ($data_ws['sq_data'] as $obj) {
		  ...
		  $q.=" INSERT INTO SISGESPATMI.TH_TRACKER (IDTRACKER,IDPNPCENTRAL,IDPROVEEDOR,IDALTERNO,LATITUD,LONGITUD,ALTURA,VELOCIDAD,FECHALOC,FECHAREG,FECHASYNC,FLGPROC,FLGUSO,FLGOK)
				VALUES(SISGESPATMI.USEQ_THTRACKER_IDTRACKER.NEXTVAL, '".$dada['id']."', 1, '".$dada['idradio']."', ".$dada['latitud'].", ".$dada['longitud'].", ".(int)$dada['altura'].", ".(($dada['velocidad']!='')?$dada['velocidad']:'0').", TO_DATE('".$dada['fechaloc']."', 'YYYY-MM-DD HH24:MI:SS'), TO_DATE('".$dada['fechareg']."', 'YYYY-MM-DD HH24:MI:SS'), SYSDATE,0,0,".((substr(@$dada['fechaloc'], 0, 13) == substr(@$dada['fechareg'], 0, 13))?1:0)."); \n";
                ...
                , HORALOC = TO_CHAR(TO_DATE('".$dada['fechaloc']."','YYYY-MM-DD HH24:MI:SS'), 'HH24MISS')
         		WHERE IDPNPRADIO = '".$dada['idradio']."' AND IDPROVEEDOR = 1 AND FLGACTIVO = 1; \n";
			}

		  

		  
		  if(($cnt_q%$md_num == $md_num-1) || ($cnt_q == $tot_num -1)){
		  	$q = $q."\n COMMIT; END;\n";
		  ...
		  if($id_location > $ultimo_id){
  		  ...
  		  $det_error = $ApiSync->get_error();
  		  if(count($det_error)>0){
  		  	$resp['error'] = $det_error;
  		  }
...
function getID(){
	$ws = @file_get_contents('http://192.168.11.23/ws_vh_mininter/Api?_sKey=RDO_X1485PU75634&_sAcc=dta_rdo_locid');
	return $ws;
}
...
if($acc == 'sync'){
	...
	if((int)@$activar['VALOR'] > 0){
		...
		if(((int)@$activar['VALOR'] == 1 && $ApiSync->get_ip()=='::1') || ((int)@$activar['VALOR'] == 2)){
			$parametro = $ApiSync->get_parametro(9);
	...
	if((int)@$activar['VALOR'] > 0){
		...
		if(((int)@$activar['VALOR'] == 1 && $ApiSync->get_ip()=='::1') || ((int)@$activar['VALOR'] == 2)){
			$parametro = $ApiSync->get_parametro(10);
