...
/* End callback wrappers */
	...
	var file = null;
	  ...
	  if (files!=null) //HTML5
	  {
	    ...
	    {
	    on_error(upload_num, { name: "InternalError", message: "Ocurrió un error al subir archivo" });
	    return;
	    }
	...
	//if certain information cannot be obtained because of a lack of support via javascript, these checks will return as valid by default, pending server-side checks after uploading
	  ...
	  {
	  on_error(upload_num, { name: "InvalidFileExtensionError", message: "Formato no permitido" });
	  return;
	  }

	  if (allowed_types.length > 0 && !validateFileMimeType(allowed_types, file))
	  {
	  on_error(upload_num, { name: "InvalidFileTypeError", message: "Formato no permitido" });
	  return;
	  }

	  if (max_file_size > 0 && !validateFileSize(max_file_size, file))
	  {
	  on_error(upload_num, { name: "MaxFileSizeError", message: "Archivo muy pesado" });
	  return;
	  }
	...
	function uploadFile(upload_num)
	{
	    ...
	    if (files[upload_num]!=undefined && files[upload_num]!=null)
	    {
	      ...
	      var ajax_xhr = $.ajaxSettings.xhr();
	        ...
	        var ajax_settings = { url: ajax_url, data: formData, type: 'post', cache: false, xhrFields: xhrFields, beforeSend: function(jqXHR) {
	          ...
	          on_error(upload_num, { name: "RequestError", message: "No hay respuesta del servidor" });
	  ...
	  {
	  on_error(upload_num, { name: "UnsupportedError", message: "El navegador no soporta este método" });
	  }
	...
	function uploadFileFallback(upload_num)
	{
	  ...
	  {
	  on_error(upload_num, { name: "UnsupportedError", message: "No soporta múltiple carga de archivos" }); //it is very unlikely this error will ever be returned, if not impossible
	  }
...
simpleUpload.iframeCallback = function(data){
    ...
    var id = data.id;
      ...
      var converted_data = simpleUpload.convertDataType(simpleUpload.iframes[id].expect, data.type, data.data);
        ...
        {
        simpleUpload.iframes[id].error("No hay respuesta del servidor");
        }
            ...
            var id = data.id;
              ...
              if (id in simpleUpload.iframes) //id is valid
              {
                ...
                var converted_data = simpleUpload.convertDataType(simpleUpload.iframes[id].expect, data.type, data.data);
                  ...
                  {
                  simpleUpload.iframes[id].error("No hay repuesta del servidor");
                  }
